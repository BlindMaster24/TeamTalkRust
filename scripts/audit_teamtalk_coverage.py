...
def summarize(records: Iterable[SymbolRecord]) -> dict[str, int]:
    items = list(records)
    wrapped = [r for r in items if r.wrapper_expected and r.in_src]
    documented = [r for r in wrapped if r.in_docs]
    direct_tested = [r for r in wrapped if r.in_tests]
    fully_covered = [r for r in wrapped if r.in_tests and r.in_docs]
    return {
        'total_symbols': len(items),
        'symbols_in_bindings': sum(r.in_bindings for r in items),
        'runtime_candidates': sum(r.wrapper_expected for r in items),
        'wrapped_symbols': len(wrapped),
        'documented_wrapped_symbols': len(documented),
        'direct_test_hits_for_wrapped_symbols': len(direct_tested),
        'fully_covered_symbols': len(fully_covered),
...
def render_markdown(records: list[SymbolRecord], summary_map: dict[str, int]) -> str:
    ...
    lines = [
        ...
        '',
        'Important:',
        '- `wrapped_symbols` means a `TT_*` runtime candidate has a direct hit in the high-level Rust source.',
        '- `direct_test_hits_for_wrapped_symbols` means tests mention that exact `TT_*` symbol name directly.',
        '- `fully_covered_symbols` means direct hits exist in source, tests, and docs for the same symbol.',
        '- `missing_tests` is a strict direct-symbol metric, not proof that runtime behavior is broken.',
        '',
        '## Summary',
        '',
    ]
...
def render_text(records: list[SymbolRecord], summary_map: dict[str, int]) -> str:
    ...
    lines = [
        ...
        '',
        'Important:',
        '- wrapped_symbols = direct TT_* hit in high-level Rust source',
        '- direct_test_hits_for_wrapped_symbols = direct TT_* hit in tests',
        '- fully_covered_symbols = direct TT_* hit in source, tests, and docs',
        '- missing_tests is a strict direct-symbol metric, not proof of broken runtime behavior',
        '',
    ]
    for key, value in summary_map.items():
        lines.append(f'{key}: {value}')
