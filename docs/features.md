...
```toml
[dependencies]
teamtalk = { version = "2.0.0", features = ["dispatch", "async"] }
```
...
- Typestate Facade: Use `TeamTalk<state::Idle>` to enforce correct initialization order (`connect` -> `login`) at compile time.
